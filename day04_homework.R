...
answer_2 <- gapminder %>% arrange(desc(gdpPercap))
...
answer_3 <- filter(gapminder, year == most_recent_year)
answer_3 <- select(answer_3, country, pop)

# 4. Answer question 4 again, but this time USE the pipe (|>). Save the
# result to answer_4
answer_4 <- gapminder |> filter(year == max(year)) |> select(country, pop)
